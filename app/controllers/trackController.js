...
async function setCurrentRev(req, res, next) {
  try {
    ...
    if (req.body.setCurrentTrackRev === '') { // set to latest version
      const rs = await core.track.queryLatestRevision(req.params.trackId)

      await core.track.setCurrentRevById(req.params.trackId, rs.rows[0].trackRevId)

      res.status(200).json({
        trackId: req.params.trackId,
        newCurrentTrackRev: rs.rows[0].trackRevId,
        result: `Current track revision for track ${req.params.trackId} set to ${rs.rows[0]
          .trackRevId}`
      })
    }

    await core.track.setCurrentRevById(req.params.trackId, req.body.setCurrentTrackRev)
    res.status(200).json({
      trackId: req.params.trackId,
      newCurrentTrackRev: req.body.setCurrentTrackRev,
      result: `Current track revision for track ${req.params.trackId} set to ${req.body
        .setCurrentTrackRev}`
    })
    return next()
  } catch (e) {
...
async function getById(req, res, next) {
  try {
    res.status(200).json({
      trackId: req.params.trackId,
      track: await core.track.getById(req.params.trackId)
    })
    next()
  } catch (e) {
    return next(e)
...
async function updateTrack(req, res, next) {
  ...
  try {
    const updatedTrack = await core.track.updateTrack(trackId, name, descr, statusId)
    res.status(200).json({
      data: updatedTrack
    })
    return next()
