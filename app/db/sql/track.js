...
async function setCurrentRevById(trackId, trackRevId) { // set the current active track Revision Id
  const rs = await knex.raw(`
...
async function getCurrentRevById(trackId) { // set the current active track Revision Id
  const rs = await knex.raw(`
...
async function getTrack(trackId, overrideRevision) {
  const trackRevId = overrideRevision ||
...
async function add({ name, descr, entityTypeName, campaignRevId, trackStatusId }) {
  return (await knex
    .returning('*')
