...
module.exports = () => {
  return {
    preParser: {
      lucidColletion: {
        _connections: {
          // arrows that connect silos together are processed and their data added here
          bySource: {}, // arrows sorted by source
          byTarget: {} // arrows sorted by target, mainly for convenience
        },
        _inputs: {
          // input arrows that provide inputs from silos to rulesets
      ...
      lucidCollectionPreped: {},
      // takes data line by line and processes it into a nice tidy object
      have(data) {
        ...
        if (typeof this.handlers[type] !== "function") {
          ...
          logger.warn(
            "Please check your Lucid diagram or extend the import library for"
          );
        ...
        const len = typeTarget.push(
          this.handlers._baseMixin(data, { containedBy })
        );
        ...
        this.handlers[type](
          data,
          typeTarget[len - 1],
          this.lucidColletion.csvHeaders
        );
      ...
      resolveFarReference(id, arrowBank) {
        ...
        const o = lc.index[id];
        const connectIds = [];
        const containers = o.containedBy.filter(
          cb => lc.index[cb.containerId].type === "ref"
        );
        containers.forEach(container => {
          const classRef = lc.index[container.containerId].classRef;
          ...
          lc.connector
            .filter(c => {
              ...
              return c.containedBy.some(cb => {
                ...
                const className = lc.index[cb.containerId].className;
                return classRef === className;
              });
            })
            .forEach(c => {
              // This gives us the class connector
      ...
      divertArrows(divertionMap, arrowBank) {
        ...
        Object.keys(arrowRegistry).forEach(id => {
          ...
          arrows.forEach(arrow => {
            ...
            if (srcObj.type === "Connector" && targetObj.type === "Connector") {
              this.resolveFarReference(arrow.srcObjectId, arrowBank).forEach(
                from => {
                  this.resolveFarReference(
                    arrow.targetObjectId,
                    arrowBank
                  ).forEach(to => {
                    const newArrow = _.cloneDeep(arrow);
                    newArrow.srcObjectId =
                      divertionMap[from.containerName][from.oldObjectId];
                    newArrow.targetObjectId =
                      divertionMap[to.containerName][to.oldObjectId];
            ...
            if (srcObj.type !== "Connector" && targetObj.type === "Connector") {
              this.resolveFarReference(arrow.targetObjectId, arrowBank).forEach(
                to => {
        ...
        Object.keys(divertionMap).forEach(instanceName => {
          Object.keys(arrowRegistry).forEach(id => {
            const arrows = arrowRegistry[id];
            arrows.forEach(arrow => {
              const srcChanged = Object.prototype.hasOwnProperty.call(
                divertionMap[instanceName],
                arrow.srcObjectId
              );
              const targetChanged = Object.prototype.hasOwnProperty.call(
                divertionMap[instanceName],
                arrow.targetObjectId
              );
              if (srcChanged && targetChanged) {
                const newArrow = _.cloneDeep(arrow);
                newArrow.srcObjectId =
                  divertionMap[instanceName][arrow.srcObjectId];
                newArrow.targetObjectId =
                  divertionMap[instanceName][arrow.targetObjectId];
                newArrows.push(newArrow);
              }
            });
          });
        ...
        newArrows.forEach(arrow => {
          if (!arrowsBySource[arrow.srcObjectId])
            arrowsBySource[arrow.srcObjectId] = [];
          arrowsBySource[arrow.srcObjectId].push(arrow);
          if (!arrowsByTarget[arrow.targetObjectId])
            arrowsByTarget[arrow.targetObjectId] = [];
      ...
      copyClassObjects(objType, divertionMap) {
        ...
        lc.ref.forEach(ref => {
          ...
          const targetClass = lc.class.filter(
            o => o.className === lc.index[ref.id].classRef
          )[0];
          Object.keys(lc.index)
            .filter(id => {
              if (lc.index[id].type !== objType) return false;
              if (!lc.index[id].containedBy) return false;
              return lc.index[id].containedBy.some(
                c => c.containerId === targetClass.id
              );
            })
            .forEach(id => {
              newId += 1;
              ...
              if (objType === "Silo") {
                ...
                if (idx !== -1) {
                  const newBoundaryContainer = {
                    containerId:
                      divertionMap[ref.name][
                        newInstance.containedBy[idx].containerId
                      ]
                  };
      ...
      arrowJoiner(bank) {
        ...
        Object.keys(this.lucidColletion[bank].bySource).forEach(src => {
          this.lucidColletion[bank].bySource[src].forEach(a => {
            a.srcObjectId = this.resolveArrowChain(
              a.srcObjectId,
              bank,
              arrowsFlat,
              true
            );
            a.targetObjectId = this.resolveArrowChain(
              a.targetObjectId,
              bank,
              arrowsFlat,
              false
      ...
      async prepare(knexConnection) {
        ...
        const res = await knexConnectionAsync
          .select("*")
          .from("core.track")
          .whereIn("name", trackNames);
        const existingTrackNames = res.map(dbTrack => dbTrack.name);

        lc.trackcontainer = lc.trackcontainer.filter(
          trackNameIn =>
            !existingTrackNames.some(
              existingTrackName => existingTrackName === trackNameIn.name
            )
        );
      ...
      handlers: {
        _baseMixin: (data, mixer = {}) => {
          // attributes here are pulled into all object types
        ...
        _arrowMangler: (data, lucidColletion) => {
          ...
          if (
            data["Source Arrow"] === "Arrow" &&
            data["Destination Arrow"] === "Arrow"
          ) {
          ...
          if (
            data["Source Arrow"] === "None" &&
            data["Destination Arrow"] === "None"
          ) {
            logger.warn(
              "Line ignored - line is not an arrow - no relationship created"
            );
            return false;
          }
          ...
          if (
            ...
            if (!connections.bySource.hasOwnProperty(arrow.srcObjectId)) {
            ...
            if (!connections.byTarget.hasOwnProperty(arrow.targetObjectId)) {
              // eslint-disable-line no-prototype-builtins, max-len
          ...
          const arrowToName =
            lucidColletion.index[data["Line Destination"]].name;

          if (
            arrowFromType !== "RuleSet" &&
            arrowFromType !== "Silo" &&
            ...
            logger.warn(
              "Arrow ignored - arrows must point from RuleSets, Silos or Channels"
            );
            logger.warn(`Found arrow that pointed from ${arrowFromType}`);
            return false;
          }
          const arrowToType =
            lucidColletion.index[data["Line Destination"]].type;
          if (
            arrowToType !== "RuleSet" &&
            arrowToType !== "Silo" &&
            arrowToType !== "Channel" &&
            ...
            logger.warn(
              "Arrow ignored - arrows must point to RuleSets, Silos or Channels"
            );
          ...
          if (
            arrowFromType === "Silo" &&
            arrowToType !== "RuleSet" &&
            arrowToType !== "Silo" && // This is now allowed, shorthand for autoPass rule
            arrowToType !== "Connector" && // This is now allowed, shorthand for autoPass rule
            arrowToType !== "Channel"
          ) {
            logger.error(
              `Arrow points from a ${arrowFromType} (${arrowFromName}) to a ${arrowToType} (${arrowToName})`
            );
            return false;
          }
        ...
        _arrowManglerInput: (data, lucidColletion) => {
          ...
          if (
            ...
            logger.warn(
              "Input arrow ignored - direction is ambigous - it should point to the rule"
            );
            return false;
          }
          ...
          if (
            ...
            logger.warn(
              "Line ignored - line is not an arrow but its marked as an input - no relationship created"
            );
            return false;
          }
          ...
          if (
            ...
            if (!inputs.bySource.hasOwnProperty(arrow.srcObjectId)) {
              // eslint-disable-line no-prototype-builtins, max-len
              inputs.bySource[arrow.srcObjectId] = [];
            }
            inputs.bySource[arrow.srcObjectId].push(arrow);
            // For convenience, do the reverse. Will make it easier to search later...
            if (!inputs.byTarget.hasOwnProperty(arrow.targetObjectId)) {
              // eslint-disable-line no-prototype-builtins, max-len
          ...
          if (arrowFromType !== "Silo" && arrowFromType !== "Connector") {
            logger.warn(
              "Input arrow ignored - input arrows must point from Silos"
            );
            logger.warn(
              `Found input arrow that points from a ${arrowFromType}`
            );
            return false;
          }
          const arrowToType =
          ...
          if (arrowToType !== "RuleSet" && arrowToType !== "Connector") {
            logger.warn(
              "Input arrow ignored - input arrows must point to RuleSets or connector"
            );
        ...
        silo: (data, target) => {
          ...
          target.name = arrSiloLabel[0];
          target.descr = arrSiloLabel[1];
        },
    ...
    dbBuilder: {
      ...
      async haveDepartment(department) {
        const res = await this.knexConnectionAsync
          .select("*")
        ...
        return (await this.knexConnectionAsync
          .insert(
            [
              {
      ...
      async haveCampaign(campaign, parentDepartmentId) {
        const res1 = await this.knexConnectionAsync
          .select("*")
          .from("core.campaign")
          .where({
            name: campaign.name
        ...
        */
          dbCampaignId = (await this.knexConnectionAsync
            .insert(
              [
                {
                  name: campaign.name,
      ...
      async haveTrack(track, containingObjectId) {
        ...
        const res2 = await this.knexConnectionAsync
          .select("*")
          .from("core.trackRev")
          .where({
            trackId: dbTrackId
        ...
        if (res2.length) {
          ...
          trackRevId = false;
          logger.warn(
            `Track already exists with name "${track.name}" - not re-importing`
          );
        } else {
          trackRevId = (await this.knexConnectionAsync
            .insert(
              [
                {
                  trackId: dbTrackId,
      ...
      async haveSilo(silo, lucidColletion) {
        ...
        silo.containedBy.forEach(containedBy => {
          if (
            lucidColletion.index[containedBy.containerId].type ===
            "trackContainer"
          ) {
      ...
      async addStep(silo, ruleSet, lucidColletion, boundary) {
        ...
        if (lucidColletion._inputs.byTarget[ruleSet.id]) {
          ...
          const collectData = arrow => {
            return {
              ...
              ...Object.keys(arrow.data)
                .filter(key => lucidColletion.csvHeaders.includes(key))
                .reduce((obj, key) => {
          ...
          if (lucidColletion._inputs.byTarget[ruleSet.id].length === 1) {
            // single arrow mode
            ruleSet.ruleParams = collectData(
              lucidColletion._inputs.byTarget[ruleSet.id][0]
            );
          } else {
        ...
        let priority = 100;
        if (lucidColletion._connections.byTarget[ruleSet.id]) {
          // protect from source rule
          lucidColletion._connections.byTarget[ruleSet.id].forEach(
            async arrow => {
              const v = parseInt(arrow.label, 10);
              if (boundary) {
                // special mode - inherit priorities from boundary container instead
                if (arrow.srcObjectId === boundary.id) {
                  priority = isNaN(v) ? 100 : v;
                }
        ...
        await this.knexConnectionAsync
          .insert(
            [
              {
      ...
      async make(lucidColletion, knexConnection) {
        ...
        // TODO: This lot needs a major refactor but fot now it works

        for (const department of lucidColletion.department) {
          // eslint-disable-line no-restricted-syntax, max-len
          department.dbId = await this.haveDepartment(department); // eslint-disable-line no-await-in-loop, max-len
        }
        ...
        for (const ruleSet of lucidColletion.ruleset) {
          ...
          ruleSet.dbId = ruleSet.ruleSetId
            ? ruleSet.ruleSetId
            : await this.haveRuleSet(ruleSet); // eslint-disable-line no-await-in-loop
        }

        if (lucidColletion.channel) {
          for (const channel of lucidColletion.channel) {
            // eslint-disable-line no-restricted-syntax, max-len
            channel.dbId = await this.haveChannel(channel); // eslint-disable-line no-await-in-loop, max-len
          }
        } else {
          logger.warn("*** Importing a track that has no channels! ***");
          logger.warn(
            "This is probably not what you want in a production system"
          );
        }

        for (const campaign of lucidColletion.campaigncontainer) {
          // eslint-disable-line no-restricted-syntax, max-len
          campaign.dbRevId = await this.haveCampaign(
            campaign,
            lucidColletion.department[0].dbId
          ); // eslint-disable-line no-await-in-loop, max-len
        }

        for (const track of lucidColletion.trackcontainer) {
          // eslint-disable-line no-restricted-syntax, max-len
          const containingObjectId = lucidColletion.campaigncontainer.find(
            campaign => {
              return campaign.id === track.containedBy[0].containerId;
            }
          ).dbRevId;
          track.dbRevId = await this.haveTrack(track, containingObjectId); // eslint-disable-line no-await-in-loop, max-len
        }

        for (const silo of lucidColletion.silo) {
          // eslint-disable-line no-restricted-syntax, max-len
          silo.dbId = await this.haveSilo(silo, lucidColletion); // eslint-disable-line no-await-in-loop, max-len
        }
        ...
        for (const silo of lucidColletion.silo) {
          ...
          if (silo.dbId) {
            ...
            if (arrPointsTo) {
              // silo actually points to something
              for (const pointerTo of arrPointsTo) {
                // eslint-disable-line no-restricted-syntax, max-len
                const targetObj =
                  lucidColletion.index[pointerTo.targetObjectId];
                ...
                if (targetObj.type === "Silo") {
                  // special shorthand condition - autoPass
        ...
        if (lucidColletion.boundary) {
          lucidColletion.boundary.forEach(b => {
            ...
            const boundedSilos = lucidColletion.silo.filter(s =>
              s.containedBy.some(cb => cb.containerId === b.id)
            );
            const arrPointsTo = lucidColletion._connections.bySource[b.id];
            if (arrPointsTo) {
              // boundary actually points to something
              for (const pointerTo of arrPointsTo) {
                // eslint-disable-line no-restricted-syntax, max-len
                const targetObj =
                  lucidColletion.index[pointerTo.targetObjectId];
        ...
        for (const silo of lucidColletion.silo) {
          // eslint-disable-line no-restricted-syntax, max-len
          if (silo.dbId) {
            if (silo.siloTypeId === "1") {
              // find source silos only
              ...
              for (const connection of connections) {
                ...
                if (ruleSet) {
                  ...
                  const isSourceRule = !lucidColletion._connections.byTarget[
                    ruleSet.id
                  ];
                  if (isSourceRule) {
        ...
        reportRecSet.rows.forEach(row => {
          if (row.trackName !== trackNameExisting) {
            ...
            op +=
              "siloId  siloName                          passList                                          failSilo\n";
          ...
          const silofmt = (siloTypeName === "Stage"
            ? `${siloName}`
            : `${siloName} (${siloTypeName})`
          ).padEnd(32, " ");
          const failSiloIdfmt =
            failSiloId === null ? "" : `${failSiloName}(${failSiloId})`;
